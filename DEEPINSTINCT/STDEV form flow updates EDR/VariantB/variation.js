...
'  </svg>';
    ...
    '  <h2 class="eg-heading">In this e-book, we outlined...</h2>'+ 
    '  <ul class="eg-list">'+ 
    '      <li>' + tickMark + 'Why the "assume breach" mentality is flawed</li>'+ 
    '      <li>' + tickMark + 'How "our" prevention stops over 99% of unknown threats </li>'+ 
    '      <li>' + tickMark + 'How "our" prevention reduces false-positive alerts to less then 0.1%</li>'+ 
    '      <li>' + tickMark + 'Why Deep Learning (DL) outperforms Machine Learning (ML)</li>'+ 
    '  </ul>'+ 
    '  <p class="eg-para"><img src="https://www.deepinstinct.com/image/bltc7189cabae58a6c8/61382d5da7dafd10aeabe438/Quotes.svg"> <span>"If EDR was enough, we would see successful ransomware attacks decreasing, not increasing"</span></p>'+ 
    '  </div>'+ 
    '  </div>'+ 
    '  <div class="eg-image-container">'+ 
    '      <img src="https://www.deepinstinct.com/image/blt8e6aac47a6e14475/6287f00e3b65ce548fba0df8/ebook-8-reasons-why-EDR-is-not-enough.jpg">'+ 
    '  </div>'+ 
    ...
    var egHeroText =''+ 
    '  <div class="free-hero-wrap" style="background-color:#2547fe;">'+ 
    '      <div class="free-hero-cont">'+ 
    '          <h1 class="eg-hero-heading">Ebook: 8 Reasons Why EDR is Not Enough</h1>'+ 
    '          <p class="eg-hero-subheading">A prevention-first approach to stopping threats must be considered!</p>'+ 
    '      </div>'+ 
    '  </div>';
    ...
    function init() {
      ...
      document.querySelector('.row-fluid-wrapper.row-depth-1.row-number-6 > .row-fluid ').innerHTML = egHeroText;
       ...
       document.querySelector('html body .row-fluid-wrapper.row-number-7 .top-sec-form form').insertAdjacentHTML('beforebegin', '<h3 class="form-title">Complete the form below to download</h3>');
       

      setTimeout(function(){
        document.querySelector('form.hs-form .hs-button').setAttribute('value', 'DOWNLOAD NOW');
      }, 2000)


      var egMainDropDown = document.querySelectorAll(".eg-main-nav-mobile .eg-menu-item > a");
      ...
      document.querySelector('.eg-logo .eg-hamburger-icon').addEventListener('click', function(){
        document.querySelector('body').classList.add('eg-mobile-menu-active');
      });
      document.querySelector('.eg-mobile-menu-section .eg-cross-icon').addEventListener('click', function(){
        document.querySelector('body').classList.remove('eg-mobile-menu-active');
      });
