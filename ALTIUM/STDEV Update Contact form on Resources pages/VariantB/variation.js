(function () {
  try {
    /* main variables */
    var debug = 0;
    var variation_name = "";

    /* all Pure helper functions */

    function waitForElement(selector, trigger, delayInterval, delayTimeout) {
      var interval = setInterval(function () {
        if (
          document &&
          document.querySelector(selector) &&
          document.querySelectorAll(selector).length > 0
        ) {
          clearInterval(interval);
          trigger();
        }
      }, delayInterval);
      setTimeout(function () {
        clearInterval(interval);
      }, delayTimeout);
    }

    var egformtop =''+ 
    '  <div class="eg-form-top">'+ 
    '      <h2 class="eg-heading">Get Altium Designer Free for 15 days!</h2>'+ 
    '      <a class="eg-logo" href="https://www.altium.com">'+ 
    '          <svg width="83" height="18" viewBox="0 0 83 18" fill="none" xmlns="http://www.w3.org/2000/svg">'+ 
    '        <path d="M21.4417 0L17.2225 17.1H21.995L26.2142 0H21.4417ZM29.9492 13.2231L31.4708 7.06154H33.4767L34.1683 4.15384H32.1625L33.2 0.138461H28.4275L24.9692 13.9154C24.6233 15.3 24.6925 16.2 25.315 16.6154C25.7992 17.0308 26.8367 17.2385 28.2892 17.1692C29.1192 17.1692 30.0183 17.1 30.9867 17.0308L31.6783 14.1231H30.9175C30.0875 14.2615 29.8108 13.9154 29.9492 13.2231ZM35.8975 4.15384L32.7158 17.1H37.4883L40.67 4.15384H35.8975ZM50.3533 4.15384L48.2783 12.7385C48.0017 13.7769 47.4483 14.2615 46.48 14.2615C45.5808 14.2615 45.235 13.7769 45.5117 12.7385L47.6558 4.15384H42.8833L40.4625 13.9846C40.1167 15.3692 40.3242 16.2692 40.9467 16.7538C41.5692 17.2385 42.3992 17.5154 43.4367 17.4461C45.1658 17.4461 46.5492 16.8231 47.4483 15.5769H47.5175L47.1717 17.1692H51.9442L55.1258 4.15384H50.3533ZM76.0142 4.5C75.3917 4.01538 74.4925 3.80769 73.3858 3.80769C72.625 3.80769 71.8642 3.94615 71.1725 4.22307C70.4117 4.5 69.72 5.05384 69.1667 5.81538C69.0283 5.12307 68.7517 4.63846 68.2675 4.29231C67.7833 3.94615 67.0225 3.80769 65.985 3.80769C64.2558 3.80769 62.8725 4.43077 61.9042 5.74615H61.835L62.25 4.15384H57.4775L54.2958 17.1H59.0683L61.2125 8.51538C61.4892 7.47692 62.0425 6.99231 63.0108 6.99231C63.91 6.99231 64.2558 7.47692 63.9792 8.51538L61.835 17.1H66.6075L68.7517 8.51538C69.0283 7.47692 69.6508 6.99231 70.55 6.99231C71.4492 6.99231 71.795 7.47692 71.5183 8.51538L69.4433 17.1H71.5183C73.04 17.1 74.4233 16.0615 74.7692 14.7461L76.5675 7.33846C76.775 5.95384 76.6367 4.98461 76.0142 4.5ZM12.7267 0C10.375 0.0692307 8.36917 1.38461 7.2625 3.39231L0 17.1H5.46417L6.77833 14.1231H10.8592L10.7208 17.1H16.185L15.5625 0H12.7267ZM11.0667 10.8692H8.23083L11.4817 3.73846L11.0667 10.8692Z" fill="white"></path>'+ 
    '        <path d="M83 15.9923C83 17.1692 82.1008 18 80.9942 18C79.8875 18 78.9883 17.1692 78.9883 15.9923C78.9883 14.8154 79.8875 13.9846 80.9942 13.9846C82.1008 14.0538 83 14.8846 83 15.9923ZM82.5158 15.9923C82.5158 15.0923 81.8933 14.4 80.9942 14.4C80.095 14.4 79.4725 15.0231 79.4725 15.9923C79.4725 16.8923 80.095 17.5154 80.9942 17.5154C81.8933 17.5846 82.5158 16.9615 82.5158 15.9923ZM80.9942 15.0231C81.4783 15.0231 81.755 15.2308 81.755 15.6462C81.755 15.9231 81.6167 16.1308 81.4092 16.2L81.8933 16.9615H81.5475L81.1325 16.2692H80.8558V16.9615H80.51V15.0231H80.9942ZM81.0633 15.9923C81.2708 15.9923 81.4092 15.8538 81.4092 15.7154C81.4092 15.5077 81.2708 15.4385 81.0633 15.4385H80.7175V16.0615H81.0633V15.9923Z" fill="white"></path>'+ 
    '      </svg>'+ 
    '      </a>'+ 
    '      <p class="eg-para"><b>Effortlessly Connect</b> With every facet of your electronics design process.</p>'+ 
    '  '+ 
    '      <p class="eg-form-heading">Start My Free Trial</p>'+ 
    '      <p class="eg-steps">(Step 1 of 2)</p>'+ 
    '  </div>';


    function init() {

      document.querySelector('.form-sidebar .form-sidebar__head').insertAdjacentHTML('beforebegin', egformtop);

    }

    /* Initialise variation */
    waitForElement(".form-sidebar .form-sidebar__head", init, 100, 25000);
  } catch (e) {
    if (debug) console.log(e, "error in Test" + variation_name);
  }
})();
