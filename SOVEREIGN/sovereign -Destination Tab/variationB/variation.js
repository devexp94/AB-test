(function () {
  try {
    ...
    var egTravelContainer = '' +
      ...
      '              <div class="eg-date-outer"><p class="eg-date"></p></div>' +
      '              <div class="eg-destination-outer"><p class="eg-destination"></p></div>' +
      '              <div class="eg-transfer-outer"><p class="eg-transfer"></p></div>' +
      '              <div class="eg-luggage-outer"><p class="eg-luggage"></p></div>' +
      '              <div class="eg-refund-outer"><p class="eg-refund">Book with Confidence</p></div>' +
      '          </div>' +
    ...
    var tickMark = '' +
      ...
      '  <path d="M15 7L8 14L5 11" stroke="#7EB583" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>' +
      '  </svg>';
    ...
    function init() {
      ...
      for (var i = 0; i < options.length; i++) {
        var label = options[i].querySelector('.MiniBasket_buttonContent').textContent;
         if(i==1)
        label='Flight '+label;
        html += '<div class="eg-label" data-index="' + i + '"><p class="eg-label-name">'+ tickMark + label + ' <span></span></p><div class="eg-dropdown"></div></div>';
      }
      html += '<div class="eg-label"><p class="eg-label-name">' + tickMark + 'Book with Confidence <span></span></p><div class="eg-dropdown"></div></div>';
      ...
      if (window.screen.width < 768) {
        document.querySelectorAll('.eg-details-container .eg-label').forEach(function (item) {
          item.addEventListener('click', function (e) {
            ...
            e.stopPropagation();
            document.querySelector('.MiniBasket_mobile .MiniBasket_options .MiniBasket_continue-content').click();
            var index = this.getAttribute('data-index');
            var targetEl = document.querySelectorAll('.MiniBasket_mobile .MiniBasket_panelContainer > div .MiniBasket_panel')[index];
      ...
      live('div.eg-details-container .eg-label:not([data-index])', 'mousedown', function () {
        window.open('https://www.sovereign.com/about/travel-with-confidence/', "_blank");
      });
      if (document.querySelector('.review-container .clearfix:nth-child(2)')) {
        
        var egReviewHeader = document.querySelector('.review-container .clearfix:nth-child(2)').innerHTML;

        document.querySelector('.hero-outside.container.css-force-display .tag-list ').insertAdjacentHTML('afterend', '<div class="eg-review-section"> '+ egReviewHeader+'</div>');
      }
    ...
    waitForElement('.CheckAvailability_continue[data-availability="open"]', init2, 100, 35000);
  } catch (e) {
