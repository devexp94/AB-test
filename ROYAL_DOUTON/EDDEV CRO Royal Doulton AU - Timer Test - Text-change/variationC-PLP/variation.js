...
'  </svg>';
    ...
    '          <h2 class="banner-heading">GOLDEN </br>WEEK </br>OFFER </br><span>NOW ON</span></h2>'+ 
    '      </div>'+ 
    '      <div class="eg-promo-timer">'+ plpBackground +
    '          <p class="banner-subheading">ENJOY A FURTHER</br>10% OFF ALREADY</br>REDUCED ITEMS*</p>'+ 
    '          <h3 class="promo-heading">HURRY,</br> GOLDEN WEEK </br>OFFER ENDS IN</h3>'+ 
    ...
    function plp() {      
      live('#filter-line .filter-block .filter-item', 'click', function () {
        checkLoader();
      });
      
      engine();
 live('.eg-mid-season-tile-plp','click',function(){
    ...
    function checkLoader(){
      var interval = setInterval(function () {
        if (document.querySelector('#rf_products_loading_overlay') && document.querySelector('#rf_products_loading_overlay').style.display == 'none') {
          clearInterval(interval);
          setTimeout(function () {
            engine();
          }, 500);
